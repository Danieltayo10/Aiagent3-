...
def send_sms_background(to_number: str, body: str):
    try:
        ...
        requests.post(url, data=payload, auth=(os.getenv("TWILIO_ACCOUNT_SID"), os.getenv("TWILIO_AUTH_TOKEN")))
...
async def query_agent(request: QueryRequest, background_tasks: BackgroundTasks, user_id: int = Depends(get_user_id)):
    ...
    # -----------------------------
    # Check ingest status first
    # -----------------------------
    status = get_status(user_id)
    if status != "completed":
        return {"answer": f"Document ingestion in progress ({status}). Please try again in a few seconds."}

    query = request.query
    index = get_index(user_id)

    if index.ntotal == 0:
        return {"answer": "No embeddings found. Wait for ingestion to complete."}
